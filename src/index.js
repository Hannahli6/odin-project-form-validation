...
const checkForms = () => {
  ...
  const forms = [emailValidState, countryValidState, zipCodeValidState, passwordValidState, confirmPasswordValidState];
...
const getZipCodeValidState = () =>{
  ...
  return {
    type: "zip-code",
    isValid: postalCodes.validate(countryCode, postalCode) === true? true : false,
...
const getPasswordValidState = () => {
  ...
  return {
    type: "password",
    isValid: isPasswordLengthValid && hasUpperCase(password) && hasLowerCase(password) && hasNumericDigit,
    errorMsg: "6 - 20 charc that contain at least 1 numeric digit, 1 uppercase and 1 lowercase letter",
  };
};
